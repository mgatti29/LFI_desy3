...
if __name__ == '__main__':
    ...
    #xlist = np.arange(len(runstodo))
    #pool.map(partial(make_maps,input_i=runstodo), xlist)
    ...
    while run_count<len(runstodo):
        ...
        if (run_count+comm.rank)<len(runstodo):
            try:
                make_maps(run_count+comm.rank,runstodo)
            except:
